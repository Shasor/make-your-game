...
export class Collision extends System {
    ...
    handleRectangleCollisions(entity, entities) {
        ...
        if (this.checkRectCollision(
            position.x, position.y, visual.width, visual.height,
        ...
        if (input.attack1 || input.attack2 || input.attack3 || input.magicAttack || input.arrowShoot) {
            for (const entity of entities) {
                ...
                if ((input.attack1 || input.attack2 || input.attack3) &&
                    distance <= hitbox.meleeRadius + enemyHitbox.collisionRadius) {
                    enemyHealth.takeDamage(25);
                }

                if ((input.magicAttack || input.arrowShoot) &&
                    distance <= hitbox.rangedRadius + enemyHitbox.collisionRadius) {
